...
void Blink(){
	printf("Blink");
	/*digitalWrite(LED_BUILTIN, HIGH);   //bật đèn led sán
  ​	delay(1000);               // dừng chương trình trong 1 giây 
  ​	digitalWrite(LED_BUILTIN, LOW);    // tắt đèn le
  ​	delay(1000);               // dừng chương trình trong 1 giây
	*/
}
...
{ 
	...
	for(int i = 0; i<3; i++ )
	{
		Blink();//nhay led thay bằng in ra Blink 3 lần
	}
	}
   while(continu == 1){    
		...
		if(strcmp(buffer, "OFF")==0 ||strcmp(buffer, "off")==0)
		{
			printf("\ntat den\n");
